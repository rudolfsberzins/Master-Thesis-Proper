...
def main():
    ...
    gen_model = pred.make_w2v_model(clf.gen_real, 'yeast_gen_real')
    both_ents_model = pred.make_w2v_model(clf.be_real, 'yeast_both_ents')
    strict_model = pred.make_w2v_model(clf.strict_real, 'yeast_strict_real')

    for seed in random_seeds:
        strict_list_pure = pred.make_models(clf.strict_real, 'yeast_strict_real_pure_'+str(seed),
                                            prev_model=strict_model, ran_state=seed)
        strict_list_gen_mod = pred.make_models(clf.strict_real,
                                               'yeast_strict_real_gen_mod_'+str(seed),
                                               prev_model=gen_model, ran_state=seed)
        strict_list_be_mod = pred.make_models(clf.strict_real,
                                              'yeast_strict_real_be_mod_'+str(seed),
        ...
        pickle.dump(errors, open('Results/yeast_errors_pickle_'+str(seed)+'.pkl', 'wb'))
        pickle.dump(fpr, open('Results/yeast_fpr_pickle_'+str(seed)+'.pkl', 'wb'))
        pickle.dump(tpr, open('Results/yeast_tpr_pickle_'+str(seed)+'.pkl', 'wb'))
