...
def main():
    ...
    # gen_model = pred.make_w2v_model(clf.gen_real, 'drosophila_gen_real')
    # both_ents_model = pred.make_w2v_model(clf.be_real, 'drosophila_both_ents')
    # strict_model = pred.make_w2v_model(clf.strict_real, 'drosophila_strict_real')

    # for seed in random_seeds:
    #     strict_list_pure = pred.make_models(clf.strict_real, 'drosophila_strict_real_pure_'+str(seed),
    #                                         prev_model=strict_model, ran_state=seed)
    #     strict_list_gen_mod = pred.make_models(clf.strict_real,
    #                                            'drosophila_strict_real_gen_mod_'+str(seed),
    #                                            prev_model=gen_model, ran_state=seed)
    #     strict_list_be_mod = pred.make_models(clf.strict_real,
    #                                           'drosophila_strict_real_be_mod_'+str(seed),
    ...
    #     pickle.dump(errors, open('Results/drosophila_errors_pickle_'+str(seed)+'.pkl', 'wb'))
    #     pickle.dump(fpr, open('Results/drosophila_fpr_pickle_'+str(seed)+'.pkl', 'wb'))
    #     pickle.dump(tpr, open('Results/drosophila_tpr_pickle_'+str(seed)+'.pkl', 'wb'))
