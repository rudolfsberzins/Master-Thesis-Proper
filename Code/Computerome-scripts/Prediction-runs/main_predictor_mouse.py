...
def main():
    ...
    strict_model = pred.make_w2v_model(clf.strict_real, 'mouse_strict_real')

    for seed in random_seeds:
        strict_list_pure = pred.make_models(clf.strict_real, 'mouse_strict_real_pure_'+str(seed),
                                            prev_model=strict_model, ran_state=seed)
        strict_list_gen_mod = pred.make_models(clf.strict_real,
                                               'mouse_strict_real_gen_mod_'+str(seed),
                                               prev_model=gen_model, ran_state=seed)
        strict_list_be_mod = pred.make_models(clf.strict_real,
                                              'mouse_strict_real_be_mod_'+str(seed),
        ...
        pickle.dump(errors, open('Results/mouse_errors_pickle_'+str(seed)+'.pkl', 'wb'))
        pickle.dump(fpr, open('Results/mouse_fpr_pickle_'+str(seed)+'.pkl', 'wb'))
        pickle.dump(tpr, open('Results/mouse_tpr_pickle_'+str(seed)+'.pkl', 'wb'))
